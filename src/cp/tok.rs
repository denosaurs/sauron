...
pub fn hash(token: &Token) -> u64 {
  ...
  match token {
    Token::Word(w) => { "WRD".hash(&mut hasher); w.hash(&mut hasher) }
    ...
    Token::Template { raw: r, cooked: _, has_escape: _ } => { "TPL".hash(&mut hasher); r.hash(&mut hasher) }
    Token::Colon => { ":".hash(&mut hasher) }
    Token::ColonColon => { "::".hash(&mut hasher) }
    Token::BinOp(o) => { "BOP".hash(&mut hasher); o.hash(&mut hasher) }
    Token::AssignOp(o) => { "AOP".hash(&mut hasher); o.hash(&mut hasher) }
    ...
    Token::Str { value: v, has_escape: _ } => { "STR".hash(&mut hasher); v.hash(&mut hasher) }
    Token::Regex(a, b) => { "REG".hash(&mut hasher); a.hash(&mut hasher); b.hash(&mut hasher) }
    Token::Num(_) => { "NUM".hash(&mut hasher) } // check later, f32 is not an hashble value
    Token::BigInt(i) => { "BG".hash(&mut hasher); i.hash(&mut hasher) }
    Token::JSXName { name: n } => { "JXN".hash(&mut hasher); n.hash(&mut hasher) }
    Token::JSXText { raw: r } => { "JXT".hash(&mut hasher); r.hash(&mut hasher) }
    Token::JSXTagStart => { "JXS".hash(&mut hasher); }
    Token::JSXTagEnd => { "JXE".hash(&mut hasher); }
    Token::Shebang(s) => { "SHB".hash(&mut hasher); s.hash(&mut hasher) }
    Token::Error(_) => { "ERR".hash(&mut hasher); }
  };
  hasher.finish()
}
